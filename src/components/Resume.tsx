    ...
    { label: "gavin.jaynes@gmail.com", href: "mailto:gavin.jaynes@gmail.com" },
    {
      ...
      href:
        mode === "web3"
          ...
          : `https://gavinjaynes.xyz/?mode=${mode}`,
    },
...
function SectionLabel({ children }: { children: string }) {
  return (
    <p className="mt-7 mb-5 font-mono text-[10px] tracking-[0.3em] text-copy-subtle uppercase">
      {children}
    </p>
  )
}
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-1.5">
      {tech.map((t) => (
        <span
          key={t}
          className="bg-zinc-100 px-1.5 py-0.5 font-mono text-[9.5px] tracking-wide text-copy-subtle"
        >
          {t}
        </span>
      ))}
  ...
  return (
    <ul className="mt-2.5 space-y-1.5">
      {items.map((b) => (
        <li
          key={b}
          className="flex items-start gap-2 text-[12px] leading-normal text-zinc-600"
        >
          <span className="mt-[3px] shrink-0 text-[10px] text-chart-5">▸</span>
          <span>{b}</span>
        </li>
      ))}
    </ul>
...
function EntryHeader({
  ...
  return (
    <div className="mb-2 flex items-baseline justify-between gap-4">
      ...
      <p className="shrink-0 text-right font-mono text-[10px] tracking-wide text-copy-subtle">
        {period}
        {location ? ` · ${location}` : ""}
      </p>
    </div>
...
export function Resume({ mode }: { mode: SiteMode }) {
  ...
  return (
    <div className="resume-root min-h-svh bg-stone-100 py-10 print:bg-white print:py-0">
      ...
      {/* ── Page 1 ── */}
      <div className="resume-sheet mx-auto mb-8 min-h-[297mm] w-[210mm] max-w-full bg-white px-[16mm] py-[11mm] shadow-lg print:mb-0">
        {/* Header */}
        <header className="border-b border-zinc-200 pb-6">
          <p className="mb-3 font-mono text-[10px] tracking-[0.3em] text-chart-5 uppercase">
            {profileTitle} · Brisbane, AU
          </p>
          <h1 className="font-display text-[30px] leading-none font-bold tracking-tight text-zinc-900">
            GAVIN JAYNES
          </h1>
          <div className="mt-4 flex flex-wrap items-baseline gap-y-1.5">
            {contacts.map(({ label, href }, i) => (
              <span key={label} className="font-mono text-[10px] text-zinc-500">
                {i > 0 && <span className="mx-3 text-zinc-300">·</span>}
                <a href={href} className="hover:text-zinc-900">
                  {label}
                </a>
              </span>
            ))}
          </div>
          <p className="mt-5 max-w-[160mm] text-[12.5px] leading-relaxed text-zinc-600">
            {SUMMARY[mode]}
          </p>
        </header>
        ...
        <div className="space-y-7">
          {orderedExperience.map((job) => (
            <div key={job.company} className="break-inside-avoid">
              ...
              />
              <p className="text-[12px] leading-normal text-zinc-600">
                {job.intro}
              </p>
      ...
      {/* ── Page 2 ── */}
      <div className="resume-sheet mx-auto min-h-[297mm] w-[210mm] max-w-full bg-white px-[16mm] py-[11mm] shadow-lg">
        {/* Earlier roles */}
        ...
        <div className="grid grid-cols-[120px_1fr] gap-x-6 gap-y-2.5">
          {displayedSkills.map(({ label, items }) => (
            <div key={label} className="contents">
              <p className="pt-0.5 font-mono text-[10px] tracking-wide text-copy-subtle uppercase">
                {label}
              </p>
              <p className="text-[12px] leading-normal text-zinc-600">
                {items}
        ...
        <p className="mt-6 font-mono text-[9.5px] tracking-[0.2em] text-copy-subtle uppercase print:hidden">
